...
class ExponentialMovingAverageCrossStrategy(object):
    """
    A basic Exponential Moving Average Crossover strategy
    that generates two exponential moving averages (SMA), with
    default windows of 500 ticks for the short EMA and
    2,000 ticks for the long EMA.

    The strategy is "long only" in the sense it will only
    open a long position once the short EMA exceeds the long
    EMA. It will close the position (by taking a corresponding
    sell order) when the long EMA recrosses the short EMA.

    The strategy uses a rolling EMA calculation in order to
    increase efficiency by eliminating the need to call two
    full moving average calculations on each tick.
    """
    def __init__(
